...
Provides functionality to list modules within a project from the filesystem. A module is defined as a git repository (directory containing a `.git` folder) located under `~/projects/<project-name>/src/`. This capability enables users to discover and view modules for a specific project or the current project.
