...
describe('proxmox vm stop', () => {
  ...
  describe('error handling', () => {
    it('shows error when VM ID not found', async () => {
      const {error} = await runCommand(['proxmox', 'vm', 'stop', '99999'])
    ...
    it('shows error when JSON mode used without VM IDs', async () => {
      const {error} = await runCommand(['proxmox', 'vm', 'stop', '--json'])
    ...
    it('handles missing Proxmox configuration', async () => {
      ...
      const {error} = await runCommand(['proxmox', 'vm', 'stop', '100'])
  ...
  describe('JSON output structure', () => {
    it('returns proper JSON structure for single VM on success', async () => {
      const {error, result} = await runCommand(['proxmox', 'vm', 'stop', '100', '--json'])
      ...
      if (result) {
        ...
        if (result.status === 'stopped') {
          ...
          // Failure: Should have stopped/failed arrays
          expect(result).to.have.property('stopped')
          expect(result).to.have.property('failed')
          expect(result.stopped).to.be.an('array')
    ...
    it('returns proper JSON structure for multiple VMs', async () => {
      const {error, result} = await runCommand(['proxmox', 'vm', 'stop', '100', '101', '--json'])
      ...
      if (result) {
        ...
        // Verify stopped VM structure if any
        if (result.stopped.length > 0) {
          const vm = result.stopped[0]
    ...
    it('returns failed array when VM not found in JSON mode', async () => {
      const {error, result} = await runCommand(['proxmox', 'vm', 'stop', '99999', '--json'])
  ...
  describe('command invocation', () => {
    it('accepts single VM ID argument', async () => {
      ...
      if (error) {
        expect(error.message).to.match(/VM 100 not found|Failed to|PROXMOX_|already stopped/)
      }
    })

    it('accepts multiple VM ID arguments', async () => {
      const {error} = await runCommand(['proxmox', 'vm', 'stop', '100', '101', '102'])
    ...
    it('accepts --json flag', async () => {
      const {error} = await runCommand(['proxmox', 'vm', 'stop', '100', '--json'])
  ...
  describe('output format', () => {
    it('outputs progress messages in non-JSON mode', async () => {
      const {error, stdout} = await runCommand(['proxmox', 'vm', 'stop', '100'])
      ...
      if (!error) {
        expect(stdout).to.match(/Stopping VM|Successfully stopped VM|Summary/)
      }
    })

    it('does not output progress messages in JSON mode', async () => {
      const {error, stdout} = await runCommand(['proxmox', 'vm', 'stop', '100', '--json'])
      ...
      if (!error) {
        expect(stdout).to.not.include('Stopping VM')
        expect(stdout).to.not.include('Successfully stopped')
      }
    })
  })
})
