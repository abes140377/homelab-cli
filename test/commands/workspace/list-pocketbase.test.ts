...
describe('workspace list-pocketbase', () => {
  ...
  it('should show error when POCKETBASE_URL is missing', async () => {
    ...
    const {error} = await runCommand('workspace list-pocketbase')
  ...
  ;(process.env.POCKETBASE_URL ? describe : describe.skip)('with PocketBase configured', () => {
    it('runs workspace list-pocketbase with valid config', async () => {
      const {error, stdout} = await runCommand('workspace list-pocketbase')
