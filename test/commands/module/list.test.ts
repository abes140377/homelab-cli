...
describe('module list', () => {
  ...
  it('should accept workspace name as argument', async () => {
    ...
    const {error} = await runCommand('module list my-workspace');
  ...
  ;(process.env.POCKETBASE_URL ? describe : describe.skip)('with PocketBase configured', () => {
    it('runs module list with valid config', async () => {
      ...
      if (error) {
        ...
        expect(error.message).to.match(/Failed to list modules|Failed to retrieve modules|not found/);
      } else {
    ...
    it('runs module list with explicit workspace name', async () => {
      const {error, stdout} = await runCommand('module list test-workspace');
    ...
    it('formats output correctly when PocketBase is accessible', async () => {
      ...
      if (error) {
        ...
        if (stdout.includes('NAME')) {
          ...
          expect(stdout).to.include('No modules found');
        }
      }
    });

    it('displays workspace name in empty message', async () => {
      const {error, stdout} = await runCommand('module list empty-workspace');

      if (error) {
        // Connection error or workspace not found is acceptable
        expect(error.message).to.match(/Failed to list modules|Failed to retrieve modules|not found/);
      } else if (stdout.includes('No modules found')) {
